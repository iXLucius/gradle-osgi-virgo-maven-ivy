...
/**
 * The
 * <code>CachingPersistenceManagerProxy</code> adds a caching layer to the
...
class CachingPersistenceManagerProxy implements PersistenceManager {
    ...
    /**
     * Indicates whether the getDictionaries method has already been called and
     * the cache is complete with respect to the contents of the underlying
     * persistence manager.
     */
    private boolean fullyLoaded;

    /**
     * Creates a new caching layer for the given actual
     ...
     * <code>Dictionary</code> objects representing the configurations stored in
     * the underlying persistence managers. The dictionaries returned are
     * garanteed to contain the
    ...
    /**
     * Returns the dictionary for the given PID or
     * <code>null</code> if no such dictionary is stored by the underyling
     * persistence manager. This method caches the returned dictionary for
     * future use after retrieving if from the persistence manager.
     * <p>
