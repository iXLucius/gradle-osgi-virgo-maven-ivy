...
/**
 * The
 * <code>ConfigurationImpl</code> is the backend implementation of the
...
class ConfigurationImpl extends ConfigurationBase {
    ...
    /**
     * The factory PID of this configuration or
     * <code>null</code> if this is not a factory configuration.
     */
     ...
     * {@link #configure(Dictionary)} method. This counter is transient and not
     * persisted. Thus it is restarted from zero each time an instance of this
     * class is created.
     */
    ...
    /**
     * Returns the "official" bundle location as visible from the outside world
     * of code calling into the Configuration.getBundleLocation() method.
     * <p>
     * In other words: The {@link #getStaticBundleLocation()} is returned if not
     * <code>null</code>. Otherwise the {@link #getDynamicBundleLocation()} is
     * returned (which may also be
     * <code>null</code>).
     */
    ...
    /**
     * Dynamically binds this configuration to the given location unless the
     * configuration is already bound (statically or dynamically). In the case
     * of this configuration to be dynamically bound a
    ...
    boolean tryBindLocation(final String bundleLocation) {
        if (this.getBundleLocation() == null) {
            getConfigurationManager().log(LogService.LOG_DEBUG, "Dynamically binding config {0} to {1}", new Object[]{getPid(), bundleLocation});
     ...
     *
     * @param deepCopy <code>true</code> if a deep copy is to be returned.
     * @return
     */
    ...
    public void update(Dictionary properties) throws IOException {
        ...
        if (localPersistenceManager != null) {
            ...
            getConfigurationManager().log(LogService.LOG_DEBUG, "Updating config {0} with {1}", new Object[]{getPid(), newProperties});
                ...
                if (factory.addPID(getPid())) {
                    ...
                    try {
                        ...
                        getConfigurationManager().log(LogService.LOG_ERROR,
                                "Failure storing factory {0} with new configuration {1}", new Object[]{factoryPid, getPid(), ioe});
                    }
                }
            }
    ...
    // ---------- private helper -----------------------------------------------
    /**
     * Stores the configuration if it is a newly factory configuration which has
     ...
     * </pre> is persisted after
     * <code>getConfiguration</code> while
     ...
     * the two calls should be synchronized on this instance to ensure
     * configuration values and revision counter match.
     */
    long getRevision() {
        return revision;
    }
    ...
    /**
     * Returns
     * <code>false</code> if this configuration contains configuration
     * properties. Otherwise
     * <code>true</code> is returned and this is a newly creted configuration
     * object whose {@link #update(Dictionary)} method has never been called.
     */
    boolean isNew() {
    ...
    /**
     * Returns
     * <code>true</code> if this configuration has already been deleted on the
     * persistence.
     */
    boolean isDeleted() {
        return isDeleted;
