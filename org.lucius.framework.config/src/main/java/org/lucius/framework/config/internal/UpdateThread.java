...
/**
 * The
 * <code>UpdateThread</code> is the thread used to update managed services and
 * managed service factories as well as to send configuration events.
 */
public class UpdateThread implements Runnable {
    ...
    public void run() {
        for (;;) {
            ...
            try {
                ...
                configurationManager.log(LogService.LOG_DEBUG, "Running task {0}", new Object[]{task});
     ...
     * all outstanding events up to and including the termination job. All jobs
     * {@link #schedule(Runnable) scheduled} after termination has been
     * initiated will not be processed any more. This method does nothing if the
     * worker thread is not currently active.
     * <p>
     * If the worker thread does not terminate within 5 seconds it is killed by
     * calling the (deprecated)
     * <code>Thread.stop()</code> method. It may be that the worker thread may
     * be blocked by a deadlock (it should not, though). In this case hope is
     * that
     * <code>Thread.stop()</code> will be able to released that deadlock at the
     * expense of one or more tasks to not be executed any longer.... In any
     * case an ERROR message is logged with the LogService in this situation.
     */
        ...
        if (this.worker != null) {
            ...
            if (workerThread.isAlive()) {
                this.configurationManager.log(LogService.LOG_ERROR,
                        "Worker thread {0} did not terminate within 5 seconds; trying to kill", new Object[]{workerBaseName});
                workerThread.stop();
            }
        }
    }
	...
	void schedule(Runnable update) {
        synchronized (updateTasks) {
            configurationManager.log(LogService.LOG_DEBUG, "Scheduling task {0}", new Object[]{update});
