...
import org.lucius.framework.utils.export.Style.Border;
import org.lucius.framework.utils.export.Style.Square;

/**
 * Copyright: Copyright 2007 - 2016 MPR Tech. Co. Ltd. All Rights Reserved.
 * Date: 2016年8月9日 上午9:31:08 Author: zhangle Version: 1.0.0.0 Description:
 * excel导出工具类
 */
public final class ExportUtil {
    ...
    static {
        ...
        borderStyle.put("MEDIUM_DASH_DOT",
    ...
    private short getVAlignment(String vAlign) {
        ...
        short va = HSSFCellStyle.VERTICAL_TOP;
        if ("CENTER".equalsIgnoreCase(vAlign)) {
            va = HSSFCellStyle.VERTICAL_CENTER;
        } else if ("BOTTOM".equalsIgnoreCase(vAlign)) {
            va = HSSFCellStyle.VERTICAL_BOTTOM;
        } else if ("JUSTIFY".equalsIgnoreCase(vAlign)) {
            va = HSSFCellStyle.VERTICAL_JUSTIFY;
        }
        return va;
    }
     ...
     *            颜色
     */
    private void setBorderColor(HSSFCellStyle style, Square<Object> diyColor) {
        ...
        style.setRightBorderColor(
                st == null ? style.getRightBorderColor() : st);
        st = getColor(diyColor.getBottom());
        style.setBottomBorderColor(
                st == null ? style.getBottomBorderColor() : st);
    }
    ...
    private void setBorder(HSSFCellStyle style, Border border) {
        ...
        Square<String> diyStyle = border == null ? null : border.getStyle();
        style.setBorderTop(
                getBorderStyle(diyStyle == null ? null : diyStyle.getTop()));
        style.setBorderLeft(
                getBorderStyle(diyStyle == null ? null : diyStyle.getLeft()));
        style.setBorderRight(
                getBorderStyle(diyStyle == null ? null : diyStyle.getBottom()));
        style.setBorderBottom(
                getBorderStyle(diyStyle == null ? null : diyStyle.getRight()));
    }
    ...
    private HSSFRow createRow(int index, Style style, HSSFSheet sheet) {
        ...
        if (style != null) {
            if (style.getHeight() != null) {
                row.setHeight((short) (ROW_HIGHT * style.getHeight()));
            }
        }
        return row;
    }
    ...
    private HSSFCellStyle createCellStyle(int index, HSSFCellStyle style,
        ...
        if (fieldStyle != null) {
            ...
            if (fieldStyle.getWidth() != null) {
                sheet.setColumnWidth(index, COL_WIDTH * fieldStyle.getWidth());
            }
        ...
        style.setAlignment(getAlignment(
                fieldStyle == null ? null : fieldStyle.getAlign())); // 设置对齐方式
        style.setVerticalAlignment(getVAlignment(
                fieldStyle == null ? null : fieldStyle.getVAlign()));
        return style;
    }
     ...
     *            单元格所属Sheet
     */
    private void createCell(CellData cellData) {
        if (cellData == null || cellData.style == null
                || cellData.fieldBean == null || cellData.langCode == null
                || cellData.row == null || cellData.sheet == null) {
            return;
        }
        try {
            HSSFCell cell = cellData.row.createCell(cellData.index);

            String textValue = null;
            if (cellData.value == null) {
                textValue = cellData.fieldBean.getTitle(cellData.langCode);
            } else {
                ValueSet valueBean = cellData.fieldBean.getValueSet();
                if (valueBean == null) {
                    // 其它数据类型都当作字符串简单处理
                    textValue = cellData.value.toString();
                } else {
                    textValue = valueBean.value(cellData.value,
                            cellData.rowValue, cellData.langCode);
                }
            }
    ...
    private void createHeader(FieldBean[] fields, Style rowStyle,
        ...
        for (FieldBean field : fields) {
            ...
            createCell(new CellData(cellIndex,
                    createCellStyle(cellIndex, style, field.getStyle(), sheet),
                    field, null, null, langCode, header, sheet));
            cellIndex++;
        }
    }
        ...
        try {
            ...
            if (fieldName.contains(".")) {
                ...
                if (t == null) {
                    ...
                    obj = map.containsKey(fieldName) ? map.get(fieldName)
                            : null;
                } else {
     ...
     *             异常
     */
    private <T> void export(String sheetName, EntityBean entityBean,
            Collection<T> dataset) throws IllegalAccessException,
            IllegalArgumentException, InvocationTargetException,
            NoSuchMethodException, SecurityException, IOException {
        if (StringUtils.isBlank(sheetName) || entityBean == null) {
            return;
        }
        // 生成一个表格
        HSSFSheet sheet = workBook.createSheet(sheetName);
        // 设置默认列宽
        sheet.setDefaultColumnWidth(DFT_COL_WIDTH);

        FieldBean[] fields = entityBean.getFields();
        LangCode langCode = entityBean.getLangCode();
            ...
            while (it.hasNext()) {
                ...
                for (FieldBean field : fields) {
                    if (cellStyles.size() - 1 < cellIndex) {
                        ...
                        cellStyles.add(cellIndex, createCellStyle(cellIndex,
                                style, field.getValueStyle(), sheet));
                    }
                    createCell(
                            new CellData(cellIndex, cellStyles.get(cellIndex),
                                    field, getValue(t, field.getName()), t,
                                    langCode, row, sheet));
                    cellIndex++;
                }
                index++;
    ...
    private <T> void export(String sheetName, OutputStream out,
            ...
            IOException {
        if (StringUtils.isBlank(sheetName) || out == null
                || entityBean == null) {
            return;
        }
        export(sheetName, entityBean, dataset);
        workBook.write(out);
    }
     ...
     *             异常
     */
    private <T> void export(LangCode langCode, OutputStream out,
            SheetBean... beans) throws IOException {
        if (out == null || beans == null || beans.length < 1) {
            return;
        }
        for (SheetBean bean : beans) {
            try {
                export(bean.getSheetName(), EntityBean
                        .getEntityBean(bean.getTemplateStream(), langCode),
                        bean.getDatalist());
        ...
        for (int i = 0, j = beans.length; i < j; i++) {
            workBook.setSheetOrder(beans[i].getSheetName(), i);
        }
        workBook.write(out);
    }
    ...
    public static <T> void exportExcel(String sheetName, OutputStream out,
            ModuleEnum moduleEnum, LangCode langCode, Collection<T> dataset)
            throws Exception {
        if (StringUtils.isBlank(sheetName) || out == null || moduleEnum == null
                || langCode == null) {
            return;
        }
        new ExportUtil().export(sheetName, out, EntityBean.getEntityBean(
                moduleEnum.getTemplateStream(), langCode), dataset);
    }
     ...
     *             异常
     */
    public static <T> void exportExcel(LangCode langCode, OutputStream out,
            SheetBean... beans) throws Exception {
        if (langCode == null || out == null || beans == null
                || beans.length < 1) {
            return;
        }
        new ExportUtil().export(langCode, out, beans);
    }

    /**
     * Copyright: Copyright 2007 - 2016 MPR Tech. Co. Ltd. All Rights Reserved.
     * Date: 2016年8月9日 上午9:45:34 Author: zhangle Version: 1.0.0.0 Description:
     * Initialize
     */
    class CellData {
        /**
        ...
        public CellData(int index, HSSFCellStyle style, FieldBean fieldBean,
                Object value, Object rowValue, LangCode langCode, HSSFRow row,
                HSSFSheet sheet) {
            super();
            this.index = index;
            this.style = style;
