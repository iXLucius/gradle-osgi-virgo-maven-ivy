...
/**
 * The
 * <code>RankingComparator</code> may be used to maintain sorted sets or to sort
 * arrays such that the first element in the set or array is the one to use
 * first and the last elementis the one to use last.
 */
...
abstract class RankingComparator implements Comparator {
     ...
     * <code>service.ranking</code> property. This results in collections whose
     * first element has the highest ranking and the last element has the lowest
     * ranking. Thus the results of this comparator are as follows:
     * <ul>
     ...
     * Implements a comparator to sort arrays and sets according to the
     * specification of the
     * <code>service.cmRanking</code> property in the Configuration Admin
     * specification. This results in collections where the first element has
     * the lowest ranking value and the last element has the highest ranking
     * value. Order amongst elements with the same ranking value is left
     * undefined. Thus the results of this comparator are as follows:
     * <ul>
     * <li><code>&lt; 0</code> if obj1 has lower ranking than obj2</li>
