...
import org.lucius.framework.config.PersistenceManager;
import org.lucius.framework.config.file.FilePersistenceManager;
...
/**
 * The
 * <code>ConfigurationManager</code> is the central class in this implementation
 * of the Configuration Admin Service Specification. As such it has the
 * following tasks:
 * <ul>
 * <li>It is a
 * <code>BundleActivator</code> which is called when the bundle is started and
 * stopped.
 * <li>It is a
 * <code>BundleListener</code> which gets informed when the states of bundles
 * change. Mostly this is needed to unbind any bound configuration in case a
 * bundle is uninstalled.
 * <li>It is a
 * <code>ServiceListener</code> which gets informed when
 * <code>ManagedService</code> and
 * <code>ManagedServiceFactory</code> services are registered and unregistered.
 * This is used to provide configuration to these services. As a service
 * listener it also listens for {@link PersistenceManager} instances being
 * registered to support different configuration persistence layers.
 ...
 * location taken from the
 * <code>felix.cm.dir</code> framework property. If this property is not set the
 * <code>config</code> directory in the current working directory as specified
 * in the
 * <code>user.dir</code> system property is used.
 */
...
public class ConfigurationManager implements BundleActivator, BundleListener {
     ...
     */
    public static final String CM_CONFIG_DIR = "agiledev.cm.dir";
    /**
    ...
    /**
     * The map of dynamic configuration bindings. This maps the PID of the
     * dynamically bound configuration or factory to its bundle location.
     * <p>
    ...
    public void start(BundleContext bundleContext) {
        ...
        OsgiConfigManager.getInstance().start(bundleContext);
        Dictionary p = new Hashtable();
        props.put("service.pid", DefaultService.COMMON_CONFIG_ID);
        redisService = bundleContext.registerService(ManagedService.class.getName(), new DefaultService(), p);          
    }
    ...
    /**
     * Returns
     * <code>true</code> if this manager is considered active.
     */
    boolean isActive() {
        return isActive;
    }
    ...
    void setDynamicBundleLocation(final String pid, final String location) {
        if (dynamicBindings != null) {
            try {
                ...
                log(LogService.LOG_ERROR, "Failed storing dynamic configuration binding for {0} to {1}", new Object[]{pid, location, ioe});
            }
        }
    }
    ...
    /**
     * Returns the {@link ConfigurationImpl} with the given PID if available in
     * the internal cache or from any persistence manager. Otherwise
    ...
    ConfigurationImpl getConfiguration(String pid) throws IOException {
        ...
        if (config != null) {
            log(LogService.LOG_DEBUG, "Found cached configuration {0} bound to {1}", new Object[]{pid, config.getBundleLocation()});
        ...
        for (int i = 0; i < pmList.length; i++) {
            if (pmList[i].exists(pid)) {
                ...
                log(LogService.LOG_DEBUG, "Found existing configuration {0} bound to {1}", new Object[]{pid, config.getBundleLocation()});
    ...
    /**
     * Creates a regular (non-factory) configuration for the given PID setting
     * the bundle location accordingly.
     * <p>
     * This method assumes the configuration to not exist yet and will create it
     * without further checking.
     *
     * @param pid The PID of the new configuration
     * @param bundleLocation The location to set on the new configuration. This
     * may be <code>null</code> to not bind the configuration yet.
     * @return The new configuration persisted in the first persistence manager.
     * @throws IOException If an error occurrs writing the configuration to the
     * persistence.
     */
            ...
            while (configs.hasMoreElements()) {
                ...
                if (!configurationAdmin.hasPermission((String) config
                    ...
                    log(
                            ...
                            new Object[]{config.get(Constants.SERVICE_PID), configurationAdmin.getBundle().getLocation(),
                ...
                if (filter == null || filter.match(config)) {
                    ...
                    if (!cfg.isNew()) {
                        log(LogService.LOG_DEBUG, "Adding configuration {0}", new Object[]{config.get(Constants.SERVICE_PID)});
                        configList.add(cfg);
                    } else {
                        log(LogService.LOG_DEBUG, "Omitting configuration {0}: Is new", new Object[]{config.get(Constants.SERVICE_PID)});
                    }
                } else {
                    log(LogService.LOG_DEBUG, "Omitting configuration {0}: Does not match filter", new Object[]{config.get(Constants.SERVICE_PID)});
                }
            }
        }
    ...
    void locationChanged(ConfigurationImpl config, String oldLocation) {
        ...
        if (oldLocation != null && !config.isNew()) {
            ...
            log(LogService.LOG_DEBUG, "LocationChanged({0}, {1}=>{2}) scheduled", new Object[]{config.getPid(), oldLocation, config.getBundleLocation()});
        } else {
            log(LogService.LOG_DEBUG,
                    "LocationChanged not scheduled for {0} (old location is null or configuration is new)", new Object[]{config.getPid()});
        }
    }
    ...
    void fireConfigurationEvent(int type, String pid, String factoryPid) {
        ...
        if (event.hasConfigurationEventListeners()) {
            ...
            log(LogService.LOG_DEBUG, "No ConfigurationListeners to send {0} event to.", new Object[]{event.getTypeName()});
        }
    }
    ...
    /**
     * Configures the ManagedService and returns the service.pid service
     * property as a String[], which may be
     * <code>null</code> if the ManagedService does not have such a property.
     */
    private void configure(String[] pids, ServiceReference sr, ManagedService service) {
        if (pids != null) {
            if (isLogEnabled(LogService.LOG_DEBUG)) {
                log(LogService.LOG_DEBUG, "configure(ManagedService {0})", new Object[]{toString(sr)});
            }

            for (int i = 0; i < pids.length; i++) {
                ManagedServiceUpdate update = new ManagedServiceUpdate(pids[i], sr, service);
                updateThread.schedule(update);
                log(LogService.LOG_DEBUG, "ManagedServiceUpdate({0}) scheduled", new Object[]{pids[i]});
            }
        }
    }

    /**
     * Configures the ManagedServiceFactory and returns the service.pid service
     * property as a String[], which may be
     * <code>null</code> if the ManagedServiceFactory does not have such a
     * property.
     */
    private void configure(String[] pids, ServiceReference sr, ManagedServiceFactory service) {
        if (pids != null) {
            if (isLogEnabled(LogService.LOG_DEBUG)) {
                log(LogService.LOG_DEBUG, "configure(ManagedServiceFactory {0})", new Object[]{toString(sr)});
            }

            for (int i = 0; i < pids.length; i++) {
                ManagedServiceFactoryUpdate update = new ManagedServiceFactoryUpdate(pids[i], sr, service);
                updateThread.schedule(update);
                log(LogService.LOG_DEBUG, "ManagedServiceFactoryUpdate({0}) scheduled", new Object[]{pids[i]});
            }
        }
    }
     ...
     * if the
     * <code>factoryPid</code> parameter is
     * <code>null</code>.
     *
     * @param pid The PID of the new configuration object. Must not be
     * <code>null</code>.
     * @param factoryPid The factory PID of the new configuration. Not
     * <code>null</code> if the new configuration object belongs to a factory.
     * The configuration object will not be persisted if this parameter is
     * not <code>null</code>.
     * @param bundleLocation The bundle location of the bundle to which the
     * configuration belongs or <code>null</code> if the configuration is not
     * bound yet.
     * @return The new configuration object
     * @throws IOException May be thrown if an error occurrs persisting the new
     * configuration object.
     */
    ConfigurationImpl createConfiguration(String pid, String factoryPid, String bundleLocation) throws IOException {
        log(LogService.LOG_DEBUG, "createConfiguration({0}, {1}, {2})", new Object[]{pid, factoryPid, bundleLocation});
     ...
     * ConfigurationPlugin services. This may be <code>null</code> in which case
     * this method just immediately returns.
     * @param targetPid The identification of the configuration update used to
     * select the plugins according to their cm.target service property
     * @param sr The service reference of the managed service (factory) which is
     * to be updated with configuration
        ...
        for (int i = 0; i < plugins.length; i++) {
            ...
            if (plugin != null) {
                try {
                    ...
                    log(LogService.LOG_ERROR, "Unexpected problem calling configuration plugin {0}", new Object[]{toString(pluginRef), t});
                } finally {
    ...
    /**
     * Returns the
     * <code>service.pid</code> property of the service reference as an array of
     * strings or
     * <code>null</code> if the service reference does not have a service PID
     * property.
     * <p>
     ...
     * returned. Otherwise (also if the property is not set)
     * <code>null</code> is returned.
     *
     * @throws NullPointerException if reference is <code>null</code>
     * @throws ArrayStoreException if the service pid is a collection and not
        ...
        if (error instanceof ConfigurationException) {
            ...
            if (ce.getProperty() != null) {
                log(LogService.LOG_ERROR, "{0}: Updating property {1} of configuration {2} caused a problem: {3}",
                        new Object[]{toString(target), ce.getProperty(), config.getPid(), ce.getReason(), ce});
            } else {
                log(LogService.LOG_ERROR, "{0}: Updating configuration {1} caused a problem: {2}", new Object[]{toString(target), config.getPid(), ce.getReason(), ce});
            }
        } else {
            {
                log(LogService.LOG_ERROR, "{0}: Unexpected problem updating configuration {1}", new Object[]{toString(target), config, error});
            }
    ...
    /**
     * Checks whether the bundle is allowed to receive the configuration with
     * the given location binding.
     * <p>
     * This method implements the logic defined CM 1.4 / 104.4.1:
     * <ul>
     * <li>If the location is
     * <code>null</code> (the configuration is not bound yet), assume the bundle
     * is allowed</li>
     * <li>If the location is a single location (no leading "?"), require the
     * bundle's location to match</li>
     * <li>If the location is a multi-location (leading "?"), assume the bundle
     * is allowed if there is no security manager. If there is a security
     * manager, check whether the bundle has "target" permission on this
     * location.</li>
     * </ul>
     */
    boolean canReceive(final Bundle bundle, final String location) {
        if (location == null) {
            log(LogService.LOG_DEBUG, "canReceive=true; bundle={0}; configuration=(unbound)", new Object[]{bundle.getLocation()});
            return true;
            ...
            log(LogService.LOG_DEBUG, "canReceive={0}: bundle={1}; configuration={2}", new Object[]{new Boolean(hasPermission), bundle.getLocation(), location});
            return hasPermission;
        }
    }
    ...
    private abstract class ServiceHelper {
        ...
        final ServiceReference[] getServices() {
            try {
                ...
                log(LogService.LOG_ERROR, "Service selection filter is invalid to update {0}", new Object[]{config, ise});
            }
            return null;
        }
    ...
    /**
     * The
     * <code>ManagedServiceUpdate</code> updates a freshly registered
     * <code>ManagedService</code> with a specific configuration. If a
     * ManagedService is registered with multiple PIDs an instance of this class
     * is used for each registered PID.
     */
    private class ManagedServiceUpdate implements Runnable {
        ...
        public void run() {
            ...
            if (config != null) {
                log(LogService.LOG_DEBUG, "Updating configuration {0} to revision #{1}", new Object[]{pid, new Long(revision)});
                ...
                if (canReceive(serviceBundle, config.getBundleLocation())) {
                    ...
                    log(
                            ...
                            new Object[]{pid, ConfigurationManager.toString(sr), config.getBundleLocation()});
    ...
    /**
     * The
     * <code>ManagedServiceFactoryUpdate</code> updates a freshly registered
     * <code>ManagedServiceFactory</code> with a specific configuration. If a
     * ManagedServiceFactory is registered with multiple PIDs an instance of
     * this class is used for each registered PID.
     */
    private class ManagedServiceFactoryUpdate implements Runnable {
        ...
        ManagedServiceFactoryUpdate(String factoryPid, ServiceReference sr, ManagedServiceFactory service) {
            ...
            try {
                ...
                log(LogService.LOG_ERROR, "Cannot get factory mapping for factory PID {0}", new Object[]{factoryPid, ioe});
            }
        ...
        public void run() {
            ...
            if (configs == null || configs.isEmpty()) {
                ...
                for (Iterator ci = configs.entrySet().iterator(); ci.hasNext();) {
                    ...
                    log(LogService.LOG_DEBUG, "Updating configuration {0} to revision #{1}", new Object[]{cfg.getPid(), new Long(revision)});
                    ...
                    if (!canReceive(serviceBundle, cfg.getBundleLocation())) {
                        log(LogService.LOG_ERROR,
                                ...
                                new Object[]{cfg.getPid(), ConfigurationManager.toString(sr), cfg.getBundleLocation()});
                        continue;
                    }
                    ...
                    if (properties != null) {
                        log(LogService.LOG_DEBUG, "{0}: Updating configuration pid={1}", new Object[]{ConfigurationManager.toString(sr), cfg.getPid()});
    ...
    /**
     * The
     * <code>UpdateConfiguration</code> is used to update
     * <code>ManagedService[Factory]</code> services with the configuration they
     * are subscribed to. This may cause the configuration to be supplied to
     * multiple services.
     */
    private class UpdateConfiguration implements Runnable {
        ...
        public void run() {
            log(LogService.LOG_DEBUG, "Updating configuration {0} to revision #{1}", new Object[]{config.getPid(), new Long(revision)});
            ...
            if (srList != null) {
                ...
                for (int i = 0; i < srList.length; i++) {
                    ...
                    if (refBundle == null) {
                        ...
                        log(LogService.LOG_ERROR,
                                "Cannot use configuration {0} for {1}: No visibility to configuration bound to {2}",
                                new Object[]{config.getPid(), ConfigurationManager.toString(ref), configBundleLocation});
                    }
    ...
    /**
     * The
     * <code>DeleteConfiguration</code> class is used to inform
     * <code>ManagedService[Factory]</code> services of a configuration being
     * deleted.
     */
        ...
        public void run() {
            ...
            if (factoryPid != null) {
                ...
                try {
                    ...
                    log(LogService.LOG_ERROR, "Failed removing {0} from the factory {1}", new Object[]{pid, factoryPid, ioe});
                }
            }
        }
        ...
        public void run() {
            ...
            if (srList != null) {
                for (int i = 0; i < srList.length; i++) {
                    ...
                    if (srBundle == null) {
                        log(LogService.LOG_DEBUG,
                                "Service {0} seems to be unregistered concurrently (not processing)", new Object[]{ConfigurationManager.toString(sr)});
                        continue;
                    }
                    ...
                    if (wasVisible && !isVisible) {
                        ...
                        // call updated method
                        helper.provide(sr);
                        log(LogService.LOG_DEBUG, "Configuration {0} provided to {1} (new visibility)", new Object[]{config.getPid(), ConfigurationManager.toString(sr)});
                    } else {
                        // same visibility as before
                        log(LogService.LOG_DEBUG, "Unmodified visibility to configuration {0} for {1}", new Object[]{config.getPid(), ConfigurationManager.toString(sr)});
                    }
                }
            }
        }
        ...
        public void run() {
            ...
            for (int i = 0; i < listeners.length; i++) {
                if (listenerProvider[i].getState() == Bundle.ACTIVE) {
                    log(LogService.LOG_DEBUG, "Sending {0} event for {1} to {2}", new Object[]{typeName, pid, ConfigurationManager.toString(listenerReferences[i])});

                    try {
                        listeners[i].configurationEvent(event);
                    } catch (Throwable t) {
                        log(LogService.LOG_ERROR, "Unexpected problem delivering configuration event to {0}",
                                new Object[]{ConfigurationManager.toString(listenerReferences[i]), t});
                    }
                }
            }
        }
    ...
    private static class ManagedServiceTracker extends ServiceTracker {
        ...
        public Object addingService(ServiceReference reference) {
            ...
            if (service instanceof ManagedService) {
                ...
                cm.log(LogService.LOG_WARNING, "Service {0} is not a ManagedService", new Object[]{service});
                pids = null;
            }
    ...
    private static class ManagedServiceFactoryTracker extends ServiceTracker {
        ...
        public Object addingService(ServiceReference reference) {
            ...
            if (serviceObject instanceof ManagedServiceFactory) {
                ...
                cm.log(LogService.LOG_WARNING, "Service {0} is not a ManagedServiceFactory", new Object[]{serviceObject});
                pids = null;
            }
